...
运行：python3 robot_mqtt_all_in_one_task_queue.py
...
任务下发和回执按 MQTT 文档：启动 data={result:0}；完成 data={mainTaskName:任务名,result:0}。
平台逐条下发任务，内部顺序排队；不接受数组，不发布 task_finished 或自定义阶段字段。
...
class RobotMQTTService:
    ...
    def publish_json(self, topic, message):
        ...
        if not (topic == REPLY_TOPIC and message.get("method") in ("task_upload", "task_stop", "task_switch")):
...
class QueuedRobotMQTTService(RobotMQTTService):
    ...
    def _task_event(self, task, stage, result, msg=""):
        task["stage"] = stage
        method = task.get("command_method", "task_upload")
        details = {}
    ...
    def _finish_active(self, stage, result, msg):
        self._task_event(self.active_task, stage, result, msg)
        if stage == "finished" and result == 0:
            finished_message = {
                "tid": self.active_task["tid"],
                "method": "finished",
                "timestamp": now_ms(),
                "data": {
                    "robotCode": ROBOT_CODE,
                    "mainTaskName": self.active_task["mainTaskName"],
                    "result": 0,
                },
            }
            self.reply_outbox.append([finished_message, None])
            print("[QUEUE REPLY]", json.dumps(finished_message, ensure_ascii=False))
        self.active_task = None
        if stage != "finished":
