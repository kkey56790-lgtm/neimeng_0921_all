...
平台逐条下发任务，内部顺序排队；支持 data.tasks 数组，每批 tid 独立；每批全部成功完成后发送 task_completed。
...
class RobotMQTTService:
    ...
    def publish_osd(self):
        ...
        current_task = message["data"]["currentTask"]
...
def main_queue_exhausted(status):
    """底盘子任务队列及循环均结束才允许认定主任务结束；缺字段不猜测。"""
    values = (status.get("queue_size"), status.get("remai_loop"))
    return all(type(value) in (int, float) and value == 0 for value in values)


def parse_task_batch(data):
    """单任务兼容；批量格式 data.tasks=[{mainTaskName, taskCode}, ...]。"""
    if "stationCode" in data:
        if any(key in data for key in ("tasks", "mainTaskName", "taskCode")):
            raise ValueError("stationCode 不能与 tasks/mainTaskName/taskCode 混用")
        codes = data["stationCode"]
        if not isinstance(codes, list) or not codes:
            raise ValueError("data.stationCode 必须是非空编码数组")
        if len(codes) > TASK_QUEUE_LIMIT:
            raise ValueError("单批任务不能超过 {} 项".format(TASK_QUEUE_LIMIT))
        for index, code in enumerate(codes, 1):
            if not isinstance(code, str) or not code.strip():
                raise ValueError("stationCode 第 {} 项必须是非空字符串".format(index))
        return [("", code.strip()) for code in codes]
    if "tasks" in data:
        if "mainTaskName" in data or "taskCode" in data:
            raise ValueError("批量 tasks 不能与顶层单任务字段混用")
        items = data["tasks"]
        if not isinstance(items, list) or not items:
            raise ValueError("data.tasks 必须是非空数组")
    else:
        items = [data]
    if len(items) > TASK_QUEUE_LIMIT:
        raise ValueError("单批任务不能超过 {} 项".format(TASK_QUEUE_LIMIT))
    selectors = []
    for index, item in enumerate(items, 1):
        if not isinstance(item, dict):
            raise ValueError("第 {} 项必须是对象".format(index))
        if item.get("robotCode", data.get("robotCode")) != ROBOT_CODE:
            raise ValueError("第 {} 项 robotCode 不匹配".format(index))
        for key in ("mainTaskName", "taskCode"):
            if not isinstance(item.get(key), str) or not item[key].strip():
                raise ValueError("第 {} 项 {} 必须是非空字符串".format(index, key))
        selectors.append((item["mainTaskName"].strip(), item["taskCode"].strip()))
    return selectors
...
def resolve_task_batch(selectors, catalog):
    ...
    for index, (name, code) in enumerate(selectors, 1):
        ...
        matches = names if name else codes
        if len(matches) != 1 or len(codes) != 1 or (name and names != codes):
...
class QueuedRobotMQTTService(RobotMQTTService):
    def __init__(self):
        ...
        self.completed_main_status = None
    ...
    def send_reply(self, tid, method, result, **details):
        ...
        if stage == "duplicate":
            ...
            return
        data = {"result": result}
        if stage in ("started", "finished", "failed", "cancelled") and details.get("mainTaskName"):
    ...
    def publish_json(self, topic, message):
        if message.get("method") in ("osd", "task_list"):
            with self.queue_lock:
                data = dict(message.get("data", {}))
                current = dict(data.get("currentTask", {}))
                completed = self.completed_main_status
                # 底盘会保留最后完成的主任务；仅清理已确认完成那次执行的展示字段。
                # 原始 HTTP 状态仍供调度判断，巡航点信息保持原样。
                if (completed is not None
                        and current.get("main_task_name") == completed.get("main_task_name")
                        and current.get("task_time") == completed.get("task_time")
                        and _task_state(current) in ("STATE_FINISH", "STATE_FINISHED")
                        and main_queue_exhausted(current)):
                    current["main_task_name"] = ""
                    current["taskCode"] = ""
                data["currentTask"] = current
                if message.get("method") == "osd":
                    data["taskQueue"] = self.queue_snapshot()
                message["data"] = data
        return super().publish_json(topic, message)

    def publish_queue_receipt(self, tid):
        message = {"tid": tid, "method": "task_queue", "timestamp": now_ms(),
                   "data": dict(self.queue_snapshot(), result=0, robotCode=ROBOT_CODE)}
        self.reply_outbox.append([message, None])
        self.command_history[tid]["wire_reply"] = message
        print("[QUEUE REPLY]", json.dumps(message, ensure_ascii=False))

    def _task_event(self, task, stage, result, msg=""):
        task["stage"] = stage
        method = ("task_finished" if (stage == "finished" and result == 0) or
                  (stage == "cancelled" and task.get("started"))
                  else task.get("command_method", "task_upload"))
    ...
    def handle_task_upload(self, tid, data):
        ...
        self.publish_queue_receipt(tid)

    def handle_task_switch(self, tid, data):
        # 先校验新任务；不合法的切换指令不能打断当前任务。
        selectors = parse_task_batch(data)
        if len(selectors) != 1:
            raise ValueError("task_switch 仅支持一个任务，批量排队请用 task_upload")
    ...
    def _finish_active(self, stage, result, msg):
        ...
        if stage == "finished" and result == 0:
            self.completed_main_status = dict(task.get("observed_status", {}))
        self._task_event(task, stage, result, msg)
        self.active_task = None
        if stage != "finished":
            if stage != "cancelled":
                self._cancel_waiting("前一任务执行失败，取消剩余任务")
            # 底盘停止/取消只结束当前项，等待项保留并由下一轮状态检查启动。
            # 平台无需等待下一次 OSD 才能更新当前项和等待列表。
            queue_message = {
                "tid": task["tid"], "method": "task_queue", "timestamp": now_ms(),
                "data": dict(self.queue_snapshot(), result=0, robotCode=ROBOT_CODE),
            }
            self.reply_outbox.append([queue_message, None])
            print("[QUEUE REPLY]", json.dumps(queue_message, ensure_ascii=False))
        elif result == 0:
            history = self.command_history.get(task["tid"], {})
            records = history.get("tasks", [])
            if (records and all(t["stage"] == "finished" for t in records)
                    and not history.get("batch_completed")):
                history["batch_completed"] = True
                completed_message = {
                    "tid": task["tid"], "method": "task_completed",
                    "timestamp": now_ms(), "data": {"result": 0},
                }
                self.reply_outbox.append([completed_message, None])
                # 同一批重传只重放整批完成回执，不重复执行。
                history["wire_reply"] = completed_message
    ...
    def _advance_tasks(self):
        if not self.active_task and (self.queue_blocked or not self.waiting_tasks):
            return
        now = time.monotonic()
        ...
        if not self.active_task:
            ...
            if state not in TERMINAL_TASK_STATES or not main_queue_exhausted(status):
                return
        ...
        if not task["started"]:
            if same_name and (state in ("STATE_DOING", "STATE_PAUSE") or
                ...
                return
        # task_name/task_arg/task_time 属于点位执行信息，继续上报，不据此切换主任务。
        # 只有同名主任务 + FINISH + 子任务队列空 + 循环结束连续两次才确认完成。
        main_finished = (same_name and state in ("STATE_FINISH", "STATE_FINISHED")
                         and main_queue_exhausted(status))
        if main_finished:
            task["main_finish_checks"] = task.get("main_finish_checks", 0) + 1
            task["last_valid"] = now
            if task["main_finish_checks"] >= 2:
                self._finish_active("finished", 0, "主任务已完成，底盘子任务队列及循环均结束")
            return
        task["main_finish_checks"] = 0
        if same_name:
            if run_time:
                task["run_task_time"] = run_time
            if state in ("STATE_DOING", "STATE_PAUSE", "STATE_FINISH", "STATE_FINISHED"):
                task["last_valid"] = now
            ...
            if state in ("STATE_FAIL", "STATE_FAILED", "STATE_CANCEL", "STATE_CANCELED",
                ...
                # 子任务异常/停止不能宣告主任务完成，更不能推进下一主任务。
                self._unknown(task, "主任务尚未确认完成，底盘子任务处于停止或失败状态", "MAIN_TASK_NOT_FINISHED")
                return
