#!/usr/bin/env python3
"""Final protocol-terminal handling for production real inspection."""
...
class ProductionInspectionOrchestratorV3(ProductionInspectionOrchestratorV2):
    def _control(self, raw):
        try:
            ...
            if action in ("inspection.platform_task", "platform_task", "task.start"):
                try:
                    ...
                    self._publish_command_status(command_id, "rejected", {
                        "reason": "task_resolution_failed", "error": str(exc),
                    })
                    self._event("PLATFORM_TASK_REJECTED", {
                        "command_id": command_id, "error": str(exc), "request": data,
                    })
            ...
            command_id = str(message.get("command_id", message.get("tid", ""))) if "message" in locals() else ""
            self._publish_command_status(command_id, "rejected", {
                "reason": "inspection_start_rejected", "error": str(exc),
            })
            self.state = "ERROR"
    ...
    def _task_status(self, raw):
        try:
            ...
            if command_status in ("failed", "rejected", "cancelled", "canceled"):
                self.state = "ERROR" if command_status in ("failed", "rejected") else self.state
                self._event("REAL_TASK_COMMAND_" + command_status.upper(), {
                    "command_id": message.get("command_id", ""),
                    "detail": message.get("data", {}),
                })
                self._publish_status(
                    str(message.get("data", {}).get("error", "task command " + command_status)))
                return
        except Exception:
            pass
    ...
    def _start_recharge(self, resume_number):
        try:
            ...
            self._event("RECHARGE_TASK_RESOLUTION_FAILED", {
                "keywords": self.recharge_keywords, "error": str(exc),
            })
            self._publish_status(str(exc))
...
if __name__ == "__main__":
    ...
    ProductionInspectionOrchestratorV3()
    rospy.spin()
