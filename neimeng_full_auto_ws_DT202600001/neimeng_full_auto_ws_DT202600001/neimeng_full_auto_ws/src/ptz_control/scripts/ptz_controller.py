...
class PTZController:
    ...
    def _goto_and_wait(self, preset, speed, seconds, version):
        with self.condition:
            ...
            self.driver.goto_preset(self.last_preset, speed)
    ...
    def _execute(self, command):
        ...
        if action == "goto_preset":
            self._goto_and_wait(data.get("preset", "1"), data.get("speed", self.preset_speed),
                                data.get("settle_seconds", self.default_settle), version)
        elif action == "home":
            ...
            for _ in range(repeat):
                for step in steps:
                    if "preset" in step:
                        preset = str(step["preset"]).strip()
                        if not preset:
                            raise ValueError("empty preset in sweep step")
                        self._goto_and_wait(preset, step.get("speed", self.preset_speed),
                                            step.get("settle_seconds", self.default_settle), version)
                        if not self._wait(step.get("pause", 0.0), version):
                            raise InterruptedError("superseded")
                        continue
